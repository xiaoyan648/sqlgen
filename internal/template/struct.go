...
const (
	createMethod = `
	func new{{.ModelStructName}}(db *gorm.DB, opts ...sqlgen.DOOption) {{.QueryStructName}} {
...
func ({{.S}} {{.QueryStructName}}) As(alias string) *{{.QueryStructName}} { 
	{{.S}}.{{.QueryStructName}}Do.DO = *({{.S}}.{{.QueryStructName}}Do.As(alias).(*sqlgen.DO))
...
`
	relationship = `{{range .Fields}}{{if .IsRelation}}` +
		...
		`{{end}}{{end}}`
	defineMethodStruct = `type {{.QueryStructName}}Do struct { sqlgen.DO }`
...
type I{{.ModelStructName}}Do interface {
	...
	WithResult(fc func(tx sqlgen.Dao)) sqlgen.ResultInfo
	ReplaceDB(db *gorm.DB)
	ReadDB() I{{.ModelStructName}}Do
	WriteDB() I{{.ModelStructName}}Do
	As(alias string) sqlgen.Dao
	Session(config *gorm.Session) I{{.ModelStructName}}Do
	Columns(cols ...field.Expr) sqlgen.Columns
	Clauses(conds ...clause.Expression) I{{.ModelStructName}}Do
	Not(conds ...sqlgen.Condition) I{{.ModelStructName}}Do
	Or(conds ...sqlgen.Condition) I{{.ModelStructName}}Do
	Select(conds ...field.Expr) I{{.ModelStructName}}Do
	Where(conds ...sqlgen.Condition) I{{.ModelStructName}}Do
	...
	Having(conds ...sqlgen.Condition) I{{.ModelStructName}}Do
	Limit(limit int) I{{.ModelStructName}}Do
	Offset(offset int) I{{.ModelStructName}}Do
	Count() (count int64, err error)
	Scopes(funcs ...func(sqlgen.Dao) sqlgen.Dao) I{{.ModelStructName}}Do
	...
	FindInBatch(batchSize int, fc func(tx sqlgen.Dao, batch int) error) (results []*{{.StructInfo.Package}}.{{.StructInfo.Type}}, err error)
	FindInBatches(result *[]*{{.StructInfo.Package}}.{{.StructInfo.Type}}, batchSize int, fc func(tx sqlgen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*{{.StructInfo.Package}}.{{.StructInfo.Type}}) (info sqlgen.ResultInfo, err error)
	Update(column field.Expr, value interface{}) (info sqlgen.ResultInfo, err error)
	UpdateSimple(columns ...field.AssignExpr) (info sqlgen.ResultInfo, err error)
	Updates(value interface{}) (info sqlgen.ResultInfo, err error)
	UpdateColumn(column field.Expr, value interface{}) (info sqlgen.ResultInfo, err error)
	UpdateColumnSimple(columns ...field.AssignExpr) (info sqlgen.ResultInfo, err error)
	UpdateColumns(value interface{}) (info sqlgen.ResultInfo, err error)
	UpdateFrom(q sqlgen.SubQuery) sqlgen.Dao
